...
final class PackageManifestTests: XCTestCase {
    ...
    func testManifestDeclaresNoNetworkDependency() throws {
        let text = try manifestText()
        XCTAssertFalse(text.contains("../../Network"), "Scanner has no network — must not depend on Network")
    ...
    func testManifestDeclaresNoOtherFeaturePackageDependency() throws {
        let text = try manifestText()
        for forbidden in ["SettingsFeature", "../SettingsFeature", "../../Features"] {
            XCTAssertFalse(text.contains(forbidden), "ScannerFeature must not depend on \(forbidden)")
        }
    }
}
